...
app = Flask(__name__)
model = pickle.load(open("model_flight.pkl", 'rb'))
...
def predict():
    if request.method == 'POST':
        ...
        json_data = {
            "airlines":
                {
                    'Air India': 0, 'GoAir': 0, 'IndiGo': 0, 'Jet Airways': 0, 'Jet Airways Business': 0,
                    "Multiple carriers": 0, "Multiple carriers Premium economy": 0, 'SpiceJet': 0,
                    'Trujet': 0, 'Vistara': 0, 'Vistara Premium economy': 0
                },
            "sources":
                {
